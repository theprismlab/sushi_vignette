...
Details about Sushi output files, including column definitions, can be found in the accompanying spreadsheet `Sushi metadata files and column definitions.xlsx`. Columns sushi repeatedly uses to match cell line information are `depmap_id`, `lua`, `pool_id`, and `cell_set`. Groups of columns frequently used throughout the pipeline operations are `sig_cols`, variables describing each unique treatment (not including replicates), `id_cols`, variables describing a unique sample, and `cell_line_cols`, variables describing how a unique cell line is defined.
...
This vignette uses encorafenib data found on the PRISM portal page of the *MTS_SEQ004_A_NEW_PIPELINE B40_COMPOUNDS* screen.
...
select_cell_lines <- c("ACH-000219", "ACH-000810")
...
points_df <- dose_response %>%
  rowwise() %>%
  do({
    doses_orig <- get_doses_up_to_one(.$minimum_dose)
    tibble(
      depmap_id = .$depmap_id,
      doses = doses_orig,
      response = .$upper_limit + ((.$lower_limit - .$upper_limit) / (1 + (doses_orig / .$inflection)^.$slope))
    )
  }) %>%
  ungroup()

# Unique sorted tick locations for x-axis
x_axis_breaks <- sort(unique(round(points_df$doses, 10)))

# Plotting
drc_plot <- ggplot() +
  geom_line(data = smooth_df, aes(x = doses, y = response, color = depmap_id)) +
  geom_point(data = points_df, aes(x = doses, y = response, color = depmap_id)) +
  scale_x_log10(
    breaks = x_axis_breaks,
    labels = scales::label_number(accuracy = 0.0001)
  ) +
  geom_vline(data = dose_response, aes(xintercept = inflection, color = depmap_id), linetype = "dashed") +
  labs(
...
for(dataset in c("Expression", "CRISPR", "Mutation")) {
  ...
  top_hits <- biomarkers %>%
    ...
    separate(y, into = treatment_columns, sep = "::") %>%
...
biomarkers %>%
  ...
  filter(pert_name == "ENCORAFENIB") %>%
  ggplot() +
  aes(x = fct_reorder(feature, regression_coef, .fun = median), y = regression_coef, color = feature, fill = feature) +
  geom_boxplot(alpha = 0.4) +
  theme_minimal() +
  theme(legend.position = "none") +
  labs(title = "Encorafenib Biomarkers using Mutation Data Across Doses")

```

If you have AUCs in your data, here is an example of how to calculate univariate biomarkers using them.
...
input_matrix <- collapsed_l2fc %>%
  ...
  tidyr::unite(cn, treatment_columns, sep = "::") %>%
  reshape2::acast(depmap_id ~ cn, value.var = response_column, fun.aggregate = median) %>%
  transform_function()
