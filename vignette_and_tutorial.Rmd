...
Formats of input files can be found [*this will be an excel download, currently a GSheet*](https://docs.google.com/spreadsheets/d/1A_9OlCjcTVJIhe1zwQjdcVyT9K3wrL4FMv0HwimZoW4/edit?gid=0#gid=0)
...
# Originial8 doses (for ticks and points)
original_doses <- dose_response$minimum_dose * 3^(0:7)

# Generate a smooth sequence of doses (for smooth curves)
smooth_dose_seq <- function(min_dose) {
  log_doses <- seq(log10(min_dose), log10(min_dose * 3^7), length.out = 100)
  10^log_doses
}

# Create smooth dose-response curve per cell line
smooth_df <- dose_response %>%
  rowwise() %>%
  do({
    doses <- smooth_dose_seq(.$minimum_dose)
    tibble(
      depmap_id = .$depmap_id,
      doses = doses,
      response = .$upper_limit + ((.$lower_limit - .$upper_limit) / (1 + (doses / .$inflection)^.$slope)),
      ec50 = .$inflection
    )
  }) %>%
  ungroup()

# Calculate points only at original 8 doses for each cell line
points_df <- dose_response %>%
  rowwise() %>%
  do({
    doses <- .$minimum_dose * 3^(0:7)
    tibble(
      depmap_id = .$depmap_id,
      doses = doses,
      response = .$upper_limit + ((.$lower_limit - .$upper_limit) / (1 + (doses / .$inflection)^.$slope))
...
# Plotting
drc_plot <- ggplot() +
  aes(x = doses, y = response, color = depmap_id) +
  geom_line(data = smooth_df) +
  geom_point(data = points_df) +
  scale_x_log10(breaks = original_doses,
  ...
  labs(title = "Dose-Response Curves for Cell Lines", 
        x = "Doses (log10 scale)",
        y = "Response") +
  theme_minimal() +
