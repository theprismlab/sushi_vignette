...
`cell_line_cols`: columns used to identify a unique instance of a barcoded cell line  are `depmap_id`, `lua`, `pool_id`, and `cell_set`;
...
`control_cols` columns that help identify the appropriate control wells for a given treatment.
...
To generate barcode counts from FASTQ files, we use an internal pipeline. It demultiplexes the data from different flow-cells  based on index barcodes which identify the PCR plate and PCR well of origin. The pipeline outputs counts of the barcodes detected in the expertiment which is then mapped to cell line and spike-in control barcodes. Alongside this process, internal PRISM databases  are queried for experiment and cell line metadata.


To obtain the cumulative raw counts of each barcode detected in each sample across all flow cells and lanes, the **collate_fastq_reads** module (`sushi/scripts/collate_fastq_reads.R`) aggregates the counts. Barcodes are categorized as either recognized or unknown. A barcode is considered recognized if its sequence is associated with a cell line or control barcode in the PRISM collection.

Next, the **filter_counts** module (`sushi/scripts/filter_counts.R`) filters the count data to just barcodes that are supposed to have been present in the particular experimental condition. Barcodes associated with cell lines added to the condition, but not detected by the sequencer (likely due to strong killing) are imputed with a count of zero. Finally, the module removes any data flagged internall for reasons like dosing failure reported in instrument logs or cell line quality concerns based on STR profiling. 
...
@UW what commands do you mean here? Is it the next example?
...
@UW: If they dont the files for running this function, we need to either provide code to make those files or describe how to make them.
...
**Although collaborators are not able to run this module due to its requirement of internal data, they are able to see how many QC metrics are calculated through this code.**
...
@UW ctrl cols should have pcr plate or replicate plate or both in it. 
...
For the sake of the example, two cell lines were chosen: both with skin lineage, but one with a BRAF mutation and the other without.
...
@UW changing the aspect ratio of the volcano plots will be helpful. I think its okay to use facet_wrap with multiple columns to update the aspect ratio


@UW Can you add arrange(qval) when printing out the two biomarker files using head( ) below

#### Example

Here is an example of how to run the univariate biomarker function on your data. `depmap_file_path` is a path to the h5 file of public release omics data from DepMap, which can be found for download on the PRISM portal.
...
for(dataset in c("Expression", "CRISPR", "Mutation")) {
  ...
  top_hits <- biomarkers %>%
    ...
    separate(y, into = trt_cols_lfc, sep = "::") %>%
    filter(pert_name == "ENCORAFENIB") %>%
    arrange(q_val) %>%
    slice_head(n = 10)
...
@UW Please remove the lineage section if fixing the lineage plots is hard

```{r example univariate lineage boxplots, echo = T, eval = T}
# Lineage boxplots ----
biomarkers_auc %>%
  filter(feature_set == "Lineage") %>%
  separate(y, into = trt_cols_auc, sep = "::") %>%
  filter(pert_name == "ENCORAFENIB") %>%
  ggplot() +
  aes(x = fct_reorder(feature, correlation_coef, .fun = median), y = correlation_coef, color = feature, fill = feature) +
  geom_boxplot(alpha = 0.4) +
  geom_point(alpha = 0.5) +
  theme_minimal() +
  theme(legend.position = "none") +
  labs(title = "Encorafenib Lineage Enrichment", x = "Lineage")
```
