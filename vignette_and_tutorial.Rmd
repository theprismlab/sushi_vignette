...
It follows a linear structure, with each step implemented as a separate module. Each module includes: (1) a Bash script to sequentially (@UW: these bash scripts do not have an impact on execution order, that's the launch_job script) execute each pipeline step, (2) an R setup script containing the necessary parameters and inputs for the module, and (3) a core R script that defines the module’s main function (@UW: functions). All three scripts are contained in their corresponding folder under the `sushi/scripts/` directory. Common utility functions used across modules are defined in the `sushi/scripts/utils/` folder. This vignette is located in the `sushi/vignette/` folder.
...
@UW: We should add some description of what is contained in the H5 biomarker file, and how we generate it.
