...
Sample data used in this vignette is found in <this MTS 28 portal page> using the encorafenib compound.
...
where $x_t$ is mean normalized counts across technical reps and
    $x_nc$ is mean normalized counts in negative control across technical replicates and
...
### Additional details the dose-response curve fitting
...
Univariate associations between the PRISM sensitivity profiles and genomic features or genetic dependencies are computed using a simple linear regression model and omics data publicly available on the DepMap portal. 

For each log2 viability at each dose (including log2AUC), multivariate models are trained and fit using the molecular characterizations and genetic dependencies of the PRISM cell lines. The resulting importance of various features can be used to suggest potential biomarkers of compound response or to inform potential hypotheses for mechanisms of action. The dose or curve-level statistic used is noted in the “Dose” column.
